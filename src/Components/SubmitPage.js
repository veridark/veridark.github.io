  ...
  return (
    <>
      ...
      <ul>
        <li>
          <p>
            <code>Model name</code> Please write here your model name. If it doensn’t have a particular name, please
            write write the underlying models/methods used. For example, if your Authorship Verification model is based
            on a siamese BERT, you could write it as <code>Siamese BERT</code>. Or if it is an ensemble of RNNs, write{' '}
            <code>RNN Ensemble</code>, etc.
          </p>
        </li>
        <li>
          <p>
            <code>Task(s)</code> Please write here the task that your model participates in. For example, you would
            write here <code>Author Verification</code>
          </p>
        </li>
        <li>
          <p>
        ...
        <li>
          <p>
            <code>External training data</code> Yes/No if your model was trained on external data (data that is not in
            the provided datasets). Large pretrained models (such as BERT) do not count as external training data.
          </p>
        </li>
        <li>
          <p>
            <code>Model size</code> <em>[Optional]</em> Please write here the number of parameters in millions. Eg, if
            you used a BERT-base, you’d probably write <code>110M</code>.
          </p>
        </li>
        <li>
          <p>
